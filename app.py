...
# Generate dynamic CSS for selected filters
def generate_filter_css(selected_filters, prefix):
    css_rules = []
    for item in selected_filters:
        safe_key = item.replace(" ", "_").replace("-", "_")
        css_rules.append(f'button[kind="secondary"][data-testid="stButton"]:has(input[value="{prefix}_{safe_key}"]) {{ border: 2px solid var(--primary-color); background-color: var(--primary-color); color: var(--foreground-color); }}')
    return "\n".join(css_rules)

# Build dynamic CSS
filter_css = ""
filter_css += generate_filter_css(st.session_state.get("selected_job_types", []), "job_type")
filter_css += generate_filter_css(st.session_state.get("selected_verification_types", []), "verification")
filter_css += generate_filter_css(st.session_state.get("selected_player_filters", []), "player")
...
for key, default in [
    ('selected_job_types', []),
    ('selected_verification_types', []),
    ('selected_year_range', [int(min(df['creation_year'])), int(max(df['creation_year']))]),
    ('selected_player_filters', []),
    ('current_page', 1),
...
with st.expander("Filters", expanded=st.session_state.filters_expanded):
    ...
    if year_range != st.session_state.selected_year_range:
        ...
        with player_cols[i]:
            ...
            if st.button(player_option, key=f"player_{player_option}", use_container_width=True):
                if is_selected:
                    ...
                    st.session_state.selected_player_filters = [player_option]
                st.rerun()
...
with tab1:
    ...
    if not filtered_df.empty:
        ...
        sort_direction = st.selectbox("Direction", ["Ascending", "Descending"], index=0 if st.session_state.sort_direction == "asc" else 1)
        ...
        if st.session_state.sort_column in filtered_df.columns:
            ...
            if st.session_state.sort_column in ["job_name", "job_creator"]:
                filtered_df = filtered_df.sort_values(by=st.session_state.sort_column, ascending=ascending, key=lambda x: x.str.lower())
            elif st.session_state.sort_column in ["creation_date", "last_updated"]:
                filtered_df[st.session_state.sort_column] = pd.to_datetime(filtered_df[st.session_state.sort_column], errors='coerce')
        ...
        current_page = st.number_input("Go to page", min_value=1, max_value=total_pages, value=st.session_state.current_page)
        ...
        for _, row in page_df.iterrows():
            with st.container():
                ...
                with col2:
                    ...
                    with info_col2:
                        job_type_display = f"{job_type} | {max_players} players" if max_players and max_players != "30" else job_type
...
with tab2:
    ...
    if not filtered_df.empty:
        ...
        sort_column = {v: k for k, v in COLUMN_DISPLAY_NAMES.items()}.get(sort_column_display, sort_column_display)
        sort_direction = st.selectbox("Direction", ["Ascending", "Descending"], index=0 if st.session_state.sort_direction == "asc" else 1, key="table_sort_direction")
        ...
        display_df.columns = ['Job Name', 'Creator', 'Type', 'Max Players', 'Created', 'Updated', 'Verification', 'Rockstar', 'Lens']
        display_df['Job Name'] = display_df.apply(lambda row: f"<a href='{row['Rockstar']}' target='_blank'>{row['Job Name']}</a>", axis=1)
        display_df['Lens'] = display_df.apply(lambda row: f"<a href='{row['Lens']}' target='_blank'>View</a>" if pd.notna(row['Lens']) else "", axis=1)
        display_df = display_df.drop(columns=['Rockstar'])
        html_table = display_df.to_html(escape=False, index=False, classes="dataframe")
